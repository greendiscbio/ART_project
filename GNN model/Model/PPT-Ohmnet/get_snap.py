...
def get_snap(genes_file):
    ...
    kidney_specific.to_csv('GNN model/Data/PPT-Ohmnet/Graph_PPT_Ohmn/PPT-Ohmnet_tissues-kidney.edgelist', sep='\t', index=False)
    G_kidney = nx.read_edgelist('GNN model/Data/PPT-Ohmnet/Graph_PPT_Ohmn/PPT-Ohmnet_tissues-kidney.edgelist', nodetype=int, data=(('tissue', str),))
    ...
    for o in out:
        print(o)
        if 'entrezgene' in o and o['query'] not in ['ACACA', 'ADRA2B', 'ATP5F1A', 'CCN1', 'CCN2', 'DACH1', 'H2AX', 'H4C1', 'H4C11', 'H4C12', 'H4C13', 'H4C14', 'H4C15', 'H4C2', 'H4C3', 'H4C4', 'H4C5', 'H4C6', 'H4C8', 'H4C9', 'HNF1B', 'MARCHF8', 'NSD2', 'ORAI1', 'PECAM1', 'PRKN', 'TWNK', 'TXNIP']:
    ...
    nx.write_edgelist(A_kidney_relabeled, 'GNN model/Data/PPT-Ohmnet/Graph_PPT_Ohmn/AD_SNAP_PPI_kidney_'+str(len(genes)-1)+'_genes_'+str(largest)+'_nodes.edgelist')
    print(A_kidney_relabeled)
    return A_kidney_relabeled



path = 'GNN model\Data\PPT-Ohmnet/mRCC_big_pool/mrcc genes big pool.xlsx'
...
genes = data.Gene
genes = genes.unique()
np.savetxt('GNN model/Data/PPT-Ohmnet/mRCC_big_pool/mrcc_gene_list_('+str(len(genes))+'_genes).txt', genes, delimiter=',', fmt='%s') 
get_snap('GNN model/Data/PPT-Ohmnet/mRCC_big_pool/mrcc_gene_list_('+str(len(genes))+'_genes).txt')