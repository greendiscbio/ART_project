...
def compare_dataframe_with_biogris_genes(df):
    path = "Sunitinib_Avelumab+Axitinib\Graph_networks\Biogrid\Data\genes_found_new_genes.txt"
    infile = open(path, "r")
    ...
    for d in df:
        if d in rna_genes:
            included = included +1
            with open('Sunitinib_Avelumab+Axitinib\Data\Preprocessed_data\Features selected\Anova_selected_'+str(n_features)+'_genes.txt',"a") as file2:
                file2.write(str(d) + "\n")
        else:
...
path = "Sunitinib_Avelumab+Axitinib/Data/Preprocessed_data/Gene_Matrix/biogrid_included_genes_ave_axi.csv"
