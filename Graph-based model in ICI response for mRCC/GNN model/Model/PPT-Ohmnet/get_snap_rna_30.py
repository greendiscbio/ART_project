...
def get_snap(genes_file):
    ...
    for o in out:
        print(o)
        if 'entrezgene' in o and o['query'] in rna_genes:
            entrezgenes.append(int(o['entrezgene']))
            mapping[int(o['entrezgene'])] = o['query']
        else:
            no_entrezgenes.append(o['query'])
    print('Genes without entrezgene or not included in rna_genes: ' + str(len(no_entrezgenes)))
    ...
    # nx.write_edgelist(A_kidney_relabeled, 'GNN model/Data/PPT-Ohmnet/Graph_PPT_Ohmn/Second big pool/AD_SNAP_PPI_kidney_RNA_30_nodes.edgelist')
