...
path ='GNN model\Data\mRCC genes (disgenet)\string_interactions_mrcc.tsv'
data = pd.read_csv(path, delimiter='\t')
data = data[['#node1', 'node2']]
data.to_csv('GNN model/Data/mRCC genes (disgenet)/network_edges_mrcc.tsv', sep="\t")
...
final_genes.to_csv("GNN model\Data\mRCC genes (disgenet)\mrcc_protein_matrix.csv")