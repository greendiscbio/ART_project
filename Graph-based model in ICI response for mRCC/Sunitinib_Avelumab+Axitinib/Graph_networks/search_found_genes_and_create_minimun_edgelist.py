...
def create_net(i, depth_level, geneList, g, found, initial_gene, branch):
    ...
    if i < depth_level and found == False: # recursive calls
        ...
        if r.status_code == 200 and len(interactions) > 0:
            with open("Graph_networks/Biogrid/Data/genes_found.txt","a") as file:
                file.write(str(g)+"\n")
            ...
            with open("Graph_networks/Biogrid/Data/genes_not_found.txt","a") as file:
                file.write(str(g)+"\n")
    return found, branch               

def get_biogrid(geneList):
    print(geneList)
    # print(len(geneList))
    with open("Graph_networks/Biogrid/Data/biogrid_minimum.edgelist","a") as file2:
        for g in geneList:
            print(g)
    ...
    # Load network
    initial_G = nx.read_edgelist("Graph_networks/Biogrid/Data/biogrid_minimum.edgelist")  
    G = nx.Graph(initial_G)

    print(G)
    print()
    nx.write_edgelist(G, "Graph_networks/Biogrid/Data/biogrid_minimum.edgelist")

RNA_file = pd.read_csv("Data/Preprocessed_data/RNA_data.csv")
geneList = np.asarray(RNA_file.columns)
a = get_biogrid(geneList)