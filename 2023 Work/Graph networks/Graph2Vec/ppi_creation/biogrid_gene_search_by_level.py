...
import os
os.chdir('2023 Work/Graph networks/Graph2Vec/ppi_creation')
...
def get_biogrid(genes_file, genes_left_file, level):
    ...
    with open('Data/biogrid_found_genes_level_'+level+'.edgelist',"a") as file2:
        for g in genes_left:
    ...
    file2.close()
    initial_G = nx.read_edgelist('Data/biogrid_found_genes_level_'+level+'.edgelist')
    G = nx.Graph(initial_G)
    with open('Data/final_not_included_genes_level_'+str(level)+'.txt',"a") as file3:
        for g in geneList:
            if g not in G.nodes():
               file3.write(str(g)+"\n")
...
a = get_biogrid('Data/disgenet_found_genes_in_rna_total.txt', 'Data/left_genes.txt', level)
